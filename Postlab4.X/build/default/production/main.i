...
; CONFIG1
  ...
  CONFIG BOREN = ON ; Brown Out Reset Selection bits (BOR controlled by ((PCON) and 07Fh), 4 bit of the PCON register)
...
ISR:
    BTFSC ((INTCON) and 07Fh), 0 ;REVISAR EL BIT DE INTERRUPCIONES DEL PUERTO B
    CALL INT_PORTB ;IR A LA SUBRUTINA DE PORTB

    BTFSC ((INTCON) and 07Fh), 2 ; REVISAR EL BIT DE INTERRUPCIONES DEL TMR0
...
INT_PORTB:
    ...
    CALL DECREMENTO
    BCF ((INTCON) and 07Fh), 0 ;LIMPIAR BIT DE INTERRUPCIóN DEL PUERTO B
    RETURN ;REGRESAR
...
INT_TMR0:
    ...
    BCF ((INTCON) and 07Fh), 2 ; LIMPIAMOS LA BANDER DE INTERRUPCION DEL TMR0
    ...
    BTFSS ((STATUS) and 07Fh), 2 ; CHEQUEAR SE ACTIVA LA BANDERA ((STATUS) and 07Fh), 2,si el bit ((STATUS) and 07Fh), 2 es 1 la resta es 0 y se prosigue; si el BIT ((STATUS) and 07Fh), 2 es 0 se vuelve a verificar
...
INCRE_SEGS:
    ...
    BTFSS ((STATUS) and 07Fh), 2; ; CHEQUEAR SE ACTIVA LA BANDERA ((STATUS) and 07Fh), 2,SI ES 1 LA RESTA SI DIO 0
...
INCRE_DEC:
    ...
    BTFSS ((STATUS) and 07Fh), 2 ; CHEQUEAR LA BANADERA DE ((STATUS) and 07Fh), 2
...
TABLA:
 ...
 main:
    ...
    BANKSEL TRISB
    BSF TRISB,UP ;((PORTD) and 07Fh), 6 COMO ENTRADA push buttons
    BSF TRISB, DOWN ;((PORTA) and 07Fh), 7 COMO ENTRADA
    BCF TRISB, 2 ;((PORTB) and 07Fh), 0 COMO SALIDA
    BCF TRISB, 3 ;((PORTB) and 07Fh), 1 COMO SALIDA
    BCF TRISB, 4 ;((PORTB) and 07Fh), 2 COMO SALIDA
    BCF TRISB, 5 ;((PORTB) and 07Fh), 3 COMO SALIDA
    ...
    BCF ((INTCON) and 07Fh), 0 ; LIMPIAR LA BANDER DE INT ON CHANGE DE PORTB
    ...
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ; HABILTAR LAS INTERUPPCIONES GLOBALES
    BSF ((INTCON) and 07Fh), 3 ; ACTIVAR EL CAMBIO DE INTERRUPCIONES EN PORTB
    BCF ((INTCON) and 07Fh), 0 ; LIMPIAR LA BANDER DE PORTB
    BSF ((INTCON) and 07Fh), 5 ; ACTIVAR INTERRUPCION DEL TMR0
    BCF ((INTCON) and 07Fh), 2 ;BANDERA DEL TMR0
    ...
    BSF ((OSCCON) and 07Fh), 6 ; 1 en el bit 6
    BCF ((OSCCON) and 07Fh), 5 ;0 en el bit 5
    BSF ((OSCCON) and 07Fh), 4 ; 1 en el bit 4
    BSF ((OSCCON) and 07Fh), 0 ; selecciona el oscilador interno
    ...
    BCF ((OPTION_REG) and 07Fh), 3 ;SE LE ASIGNA EL PRESCALER AL TIMER, PRE EN 0
    BSF ((OPTION_REG) and 07Fh), 2 ; ((OPTION_REG) and 07Fh), 2 EN 1
    BSF ((OPTION_REG) and 07Fh), 1; ((OPTION_REG) and 07Fh), 1 EN 1
    BSF ((OPTION_REG) and 07Fh), 0 ; ((OPTION_REG) and 07Fh), 0 EN 1
    BCF ((OPTION_REG) and 07Fh), 5 ; PONER ((OPTION_REG) and 07Fh), 5 EN 0 PARA QUE OPERE COMO TIMER, Timer0 Clock Source Select bit
